...
void RANSACLocalization::callback_scan(const sensor_msgs::msg::LaserScan::SharedPtr msg){
  ...
  if(abs(scan_odom_motion[0]) > 0.03) est_diff_sum[0] += estimated[0] - current_scan_odom[0];
  if(abs(scan_odom_motion[1]) > 0.03) est_diff_sum[1] += estimated[1] - current_scan_odom[1];
