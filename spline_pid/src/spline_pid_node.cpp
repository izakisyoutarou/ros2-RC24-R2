...
init_angular:
    ...
    publisher_progress = this->create_publisher<std_msgs::msg::Float64>("move_progress", _qos);
...
void SplinePid::_publisher_callback(){
    ...
    if(max_trajectories>0){ //追従時
        ...
        // 追従進捗
        progress = (double)current_count / max_trajectories;
        msg_progress->data = progress;
        if(!is_arrived){
            publisher_progress->publish(*msg_progress); // 追従時のみ進捗を出版
        }
    ...
    publisher_can->publish(*msg_linear);
    publisher_can->publish(*msg_angular);
}
...
void SplinePid::_subscriber_callback_path(const path_msg::msg::Path::SharedPtr msg){
    ...
    if(size==msg->x.size() && size==msg->y.size() && size==msg->angle.size() && size==msg->curvature.size()){
        ...
        velPlanner_linear.pos(path->length.back(), 0.0);
        if(!is_accurate_convergence) velPlanner_angular.pos(path->angle.back(), 0.0);
...
void SplinePid::_subscriber_callback_base_control(const controller_interface_msg::msg::BaseControl::SharedPtr msg){
    if(msg->is_restart && msg->is_move_autonomous){
        max_trajectories = 0;
        velPlanner_linear.current(0.0, 0.0, 0.0);
        velPlanner_angular.current(self_pose.z, 0.0, 0.0);
        publish_is_tracking(false);
