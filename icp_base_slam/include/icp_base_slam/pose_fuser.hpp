...
public:
  PoseFuser(){}
  ~PoseFuser(){}
  void fuse_pose(const Pose &ndt_estimated, const Pose &scan_odom_motion, const Pose &predict, const double dt_scan, const PclCloud::Ptr current_cloud, const Eigen::Matrix4d transformation_matrix, Pose &estimated);
  ...
  double calculate_ndt_covariance(const Pose &ndt_estimated, Eigen::Matrix2d &ndt_cov, const Eigen::Matrix4d transformation_matrix, vector<const LaserPoint*> &current_points, vector<const LaserPoint*> &reference_points, NormalVector normal_vector);
  ...
  void calculate_motion_covariance(const Pose &scan_odom_motion, const double dt, Eigen::Matrix2d &scan_odom_motion_cov);
