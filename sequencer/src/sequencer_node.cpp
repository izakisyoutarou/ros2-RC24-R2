...
void Sequencer::callback_convergence(const controller_interface_msg::msg::Convergence::SharedPtr msg){
    ...
    if(sequence_mode == SEQUENCE_MODE::storage){
        ...
        else if(progress == n++ && !msg->spline_convergence && msg->arm_convergence && get_front_ball && std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - suction_time).count() > 5000 && (way_point == "ST0" || way_point == "ST1" || way_point == "ST2" || way_point == "ST3" || way_point == "ST4" || way_point == "ST5" || way_point == "ST6" || way_point == "ST7")){
