...

{
    ...
    publisher_pole_m0 = this->create_publisher<std_msgs::msg::String>("injection_pole_m1", _qos);
...
void Sequencer::_subscriber_callback_base_control(const controller_interface_msg::msg::BaseControl::SharedPtr msg){
    ...
    judge_convergence.spline_convergence = msg->is_wheel_autonomous;
...
void Sequencer::_recv_robot_state(const unsigned char data[2]){
    ...
    current_inject_state = state;

    if(state=="A") pole_priority_file_path = pole_priorityA_file_path;
    else if(state=="B") pole_priority_file_path = pole_priorityB_file_path;
    else if(state=="C") pole_priority_file_path = pole_priorityC_file_path;
...
void Sequencer::_recv_pole_state(const unsigned char data[11]){
    ...
    for(const auto& pole_num : pole_priority_m0){
        if(!data[pole_num] && pole_num != last_pole_num_m0){
            ...
            last_pole_num_m1 = pole_num;
            break;
        }
    }
    for(const auto& pole_num : pole_priority_m1){
        if(!data[pole_num] && pole_num != last_pole_num_m1){
            string pole{static_cast<char>(pole_num+0x41)};  //0~10をA~Kに変換
