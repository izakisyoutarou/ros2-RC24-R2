...

{
    ...
    _publisher_storage_flag = this->create_publisher<std_msgs::msg::Empty>("storage_flag", _qos);
    ...
    if(sequence_mode == SEQUENCE_MODE::storage){
        if(progress == n++){
            if(c3b_flag) {
                ...
                RCLCPP_INFO(get_logger(),"_____strage0_c5____");
                command_move_node("c5");
            }
            timer(500);
            progress++;
        }
            ...
            if(move_node == "c5"){
                if(way_point == "c5"){
                    if(c6a_flag){
                        RCLCPP_INFO(get_logger(),"_____strage2_c6a_____");
                        command_move_node("c6_a");
                    }
                    else {
                        RCLCPP_INFO(get_logger(),"_____strage2_c6b_____");
        ...
        else if(progress == n++ && msg->arm_convergence && !msg->spline_convergence){
            RCLCPP_INFO(get_logger(),"_____strage3_____");
            if(storage_flag){
                auto msg_storage_flag = std::make_shared<std_msgs::msg::Empty>();
                _publisher_storage_flag->publish(*msg_storage_flag);
                ball_pose.x = 0.f;
                ball_pose.y = 0.f;
            }
            progress++;
        }
        else if(progress == n++ && ball_pose.x != 0.f && ball_pose.y != 0.f){
            command_hand_suction_on();
            command_ball_tracking();
            progress++;
        }

        else if(progress == n++ && coord_flag){
            ...
            RCLCPP_INFO(get_logger(),"_____strage6_____");
            ...
            if(c3b_flag) {
                RCLCPP_INFO(get_logger(),"_____strage7_c3b____");
                command_move_interrupt_node("c3_b");
            }
            else if(c3a_flag){
                RCLCPP_INFO(get_logger(),"_____strage7_c3a____");
                command_move_interrupt_node("c3_a");
            }
            else if(c6a_flag){
                RCLCPP_INFO(get_logger(),"_____strage7_c6a____");
                command_move_interrupt_node("c6_a");
            }
            else {
                RCLCPP_INFO(get_logger(),"_____strage7_c6b____");
        ...
        if(progress == n++){
            ...
            if(way_point != "ST8" && !msg->spline_convergence) command_move_node("ST8");
            if(tof[2] || tof[3]){
            ...
            else if(msg->net_convergence && timer()) {
                command_net_open();
                progress -= 2;
            }
        }
        ...
        if(progress == n++){
            ...
            if(pre_sequence == SEQUENCE_MODE::transfer && tof[1]){
                RCLCPP_INFO(get_logger(),"_____silo1_transfer_____");
                command_hand_suction_on();
                command_transfer_state();
                progress++;
            } 
            else if(pre_sequence != SEQUENCE_MODE::transfer) {
                RCLCPP_INFO(get_logger(),"_____silo1_not_transfer_____");
                progress++;
            }
        }
        else if(progress == n++){
            ...
            if(priority_num == 4){
                RCLCPP_INFO(get_logger(),"_____silo5_last_____");
                command_hand_lift_silo();
                std::string silo_node = "SI" + std::to_string(silo_priority[priority_num]);
                command_move_interrupt_node(silo_node);
                progress++; 
                ...
                RCLCPP_INFO(get_logger(),"_____silo5_change______");
                priority_num++;
                progress--; 
            } 
            else {
                RCLCPP_INFO(get_logger(),"_____silo5_go_____");
                std::string silo_node = "SI" + std::to_string(silo_priority[priority_num]);
                command_move_node(silo_node);
                progress++;
            }
        }
        else if(progress == n++ && check_way_SI()){
            RCLCPP_INFO(get_logger(),"_____silo6_____");
            command_hand_suction_off();
            timer(300);
            progress++;
        } 
            ...
            //     if(loop_flag)command_sequence(SEQUENCE_MODE::storage);
            //     else{
            //         ball_num++;
            ...
            //     }
            // }
            // if(pre_sequence == SEQUENCE_MODE::transfer) {
            //     transfer_num++;
            //     if(transfer_num == 5) {
...
void Sequencer::callback_way_point(const std_msgs::msg::String::SharedPtr msg){
    ...
    if(msg->data == "coord") coord_flag = true;
}
