...
void PoseFuser::setup(vector<LineData> &_lines, const double laser_weight, const double odom_weight_liner, const double odom_weight_angler){
...
Vector3d PoseFuser::fuse_pose(Vector3d &trans, const Vector3d &current_scan_odom, double &vt, double &wt, const vector<LaserPoint> &src_points){
  init();
  if(src_points.size()==0) return current_scan_odom;
  find_correspondence(trans, src_points, current_points, reference_points);
  Vector3d laser_estimated = current_scan_odom + trans;
  ...
  Vector3d estimated = fuse(laser_estimated, laser_cov, current_scan_odom, rotate_scan_odom_motion_cov);

  //共分散合成での誤差に対応
  for(size_t i=0; i<3; i++){
    if(trans[i]==0.0) estimated[i] = current_scan_odom[i];
  }

  return estimated;
}

void PoseFuser::find_correspondence(Vector3d &trans, const vector<LaserPoint> &src_points, vector<CorrespondLaserPoint> &current_points, vector<CorrespondLaserPoint> &reference_points){
  vector<LaserPoint> global_points = transform(src_points, trans);
...
CorrespondLaserPoint PoseFuser::find_closest_vertical_point(CorrespondLaserPoint global){
  ...
  for(int i=0; i<lines.size(); i++){
    vertical_distance.x = fabs(lines[i].p_1.x - global.x);
    vertical_distance.y = fabs(lines[i].p_1.y - global.y);
    if(vertical_distance.x < distance_min){
      distance_min = vertical_distance.x;
      closest.x = lines[i].p_1.x;
      closest.y = global.y;
    }
    if(vertical_distance.y < distance_min){
      distance_min = vertical_distance.y;
      closest.x = global.x;
      closest.y = lines[i].p_1.y;
    }
  }

  for(int i=0; i<lines.size(); i++){
    if(closest.x==lines[i].p_1.x){
      closest.nx=1.0;
      closest.ny=0.0;
    }
    else if(closest.y==lines[i].p_1.y){
      closest.nx=0.0;
      closest.ny=1.0;
    }
  }
...
Matrix3d PoseFuser::calc_motion_cov(double vt, double wt){
  ...
  double wt_=wt*wt*wt*wt*wt*wt*wt*wt;
  Matrix3d C1;
  C1.setZero();
